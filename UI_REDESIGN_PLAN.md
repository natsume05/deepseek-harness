...
| P5 侧边栏 | ✅ 完成 | 选中行品牌竖条 + 语义选中底色、New Session 品牌蓝主按钮；BrandWordmark 渐变裁剪（精确提取的品牌 SVG 资产，避免偏离设计） |
| P6 动效打磨 | ✅ 完成 | 菜单/模态进出场动画、菜单项/关闭钮焦点环、动效 token 消费、全局 reduced-motion 兜底；图标集统一与骨架屏 shimmer 裁剪（边际收益低） |
| P7 总验收 | 进行中 | 本清单逐项核对；test:web replay 在 Windows 环境的工具层失败已确证与改动无关（bash/terminal 平台限制），CI（Linux）为权威信号 |

**裁剪项**：⌘K 命令面板（P4）、BrandWordmark 渐变（P5）、图标集逐枚统一与骨架屏 shimmer（P6）——均为边际收益低或偏离品牌资产，记录于此，可按需后续恢复。

## 11. 验收清单（总）

- [x] 全部新增样式消费语义 token；feature 组件无字面色（grep 抽查：唯一字面色为按钮白字 #fff，遵循 InputBar `.primary` 先例）。
- [ ] 浅/深主题下：三栏、会话、输入、侧边栏、菜单/模态/命令面板、空状态、加载态截图对比达标。（需浏览器截图，待人工/CI 补齐）
- [x] 正文对比度 ≥ 4.5:1，大字号 ≥ 3:1（两主题）。（抽查：渐变口号大字 ≥4:1、白字按钮 4.6:1、深色渐变已提亮）
- [x] `prefers-reduced-motion: reduce` 下无位移/闪烁动画残留，状态仍可读。（组件级关闭 + 全局兜底）
- [x] 键盘可完成：折叠/展开侧边栏、切换面板、操作工具卡片。（焦点环已补：菜单项、模态关闭、队列行等）
- [x] `pnpm run test:gui` 全绿（273 文件 / 3792 测试，历轮验证）；`DSH_SNAPSHOT=replay pnpm run test:web` 中 UI 相关快照全过，工具层失败为 Windows 平台限制。
- [x] 覆盖率门禁通过（定向跑 6 个改动包 1199/1200 通过，唯一失败为 coverage 模式下 shiki 异步时序 flake，单独重跑通过）。
- [x] 流式/展开热路径性能无回退（entering 属性挂载捕获 + memo 自定义比较器，邻居行零 re-render 测试保持）。
- [ ] 图标集规格统一（24px / 1.5px 描边 / 圆头）。（裁剪项，见上）
- [x] 无新增模型可见输入；无 session 事件变更；无数据层改动。
- [x] 每阶段 PR 携带对应 Agent Note（视觉风格双轨 + 本实施记录）。
